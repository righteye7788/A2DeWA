...
from SA_AET_RE import Attacker, ImageAttacker, TextAttacker
from dataset import paired_dataset2

def check_perturbation(x, x_adv, epsilon, norm='linf'):
    print(f'epsilon:{epsilon}')
    print(f'demonsion: {x.shape}, {x_adv.shape}')
    delta = x_adv - x
    delta_flat = delta.view(delta.size(0), -1)

    if norm == 'linf':
        values = delta_flat.abs().max(dim=1)[0]
    elif norm == 'l2':
        values = delta_flat.norm(p=2, dim=1)
    elif norm == 'l1':
        values = delta_flat.norm(p=1, dim=1)
    else:
        raise ValueError("norm must be 'linf', 'l2', or 'l1'")

    is_valid = values <= epsilon
    return delta, values, is_valid
...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers, device, args, config):
    ...
    for t_model, model_name in zip(t_models, args.model_list):
    ...
    img_attacker = ImageAttacker(images_normalize, device=device, embed_dim=config['embed_dim'],
                                 eps=8/255, steps=10, step_size=2/255)
    ...
    attacker = Attacker(model, img_attacker, txt_attacker, device=device)
    ...
    print('all_texts_all prepared')

    num_samples = int(0.1 * len(all_texts_all))

       #使用这些索引来选取张量中的数据
    all_texts = random.sample(all_texts_all, num_samples)

    with torch.no_grad():
        all_texts_input = txt_attacker.tokenizer(all_texts, padding='max_length', truncation=True,
                                                    max_length=max_length, return_tensors="pt").to(device)
        all_texts_output = model.inference_text(all_texts_input)
        all_txt_supervisions = all_texts_output['text_feat']
        del all_texts_output
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups,image_paths) in enumerate(data_loader):
        ...
        adv_images, adv_texts, average_loss, execuate_time = attacker.attack(images, texts, txt2img, all_txt_supervisions, device=device,
                                                max_length=max_length, scales=scales)
        
        # 检验
        # perturbation, values, is_valid = check_perturbation(images, adv_images, epsilon=2/255, norm='linf')
        # print("扰动值:", values.detach().cpu().numpy())
        # print("是否满足阈值:", is_valid)
        # print("是否全部满足:", is_valid.all().item())
        # print("values - epsilon:", (values - 2/255).detach().cpu().numpy())

        if batch_idx % 10 == 0:
            print(f'【batch:{batch_idx}/{len(data_loader)}】, average loss ========> {np.average(average_loss)}')
    ...
    # print('Attack Finished, Pating Loss Curve....')
    # utils.paint_loss(average_loss_list, len(data_loader), config['loss_curve'])
...
def eval_asr(model, ref_model, tokenizer, t_models, t_ref_models, t_tokenizers, t_test_transforms, data_loader, device, args, config):
    ...
    # result_file_path = "./result_ablation.txt"
    result_file_path = "./result_coco.txt"

    with open(result_file_path, "a") as file:
        file.write("\n")
        # file.write("device: {}".format(device) + "\n")
        file.write("扰动阈值对比(w/o importance map w/o smooth v1.5分层) eps: 8/255 \n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/Retrieval_coco_re.yaml')
    ...
    parser.add_argument('--albef_ckpt', default='/home/data/share_data/multimodal/models/others/checkpoints/mscoco.pth', type=str) 
    parser.add_argument('--tcl_ckpt', default='/home/data/share_data/multimodal/models/TCL/checkpoint_coco_finetune.pth', type=str)    
